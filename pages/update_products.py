...
def get_all_articles(selected_columns, filters, sorts, limit, offset):
    ...
    response = requests.get(get_localsystem_url()+'articles/', auth=get_auth_local(), json=params)
...
if 'selected_articles' in st.session_state and st.session_state.selected_articles:
    ...
    if st.button("Generate Content with ChatGPT"):
        ...
        for idx, (_, row) in enumerate(article_info.iterrows()):
            ...
            # update the article
            update_response = requests.put(f"{get_localsystem_url()}articles/{article_id}", auth=get_auth_local(), json=payload)
...
def get_article_by_id(article_id):
    response = requests.get(f"{get_localsystem_url()}articles/{article_id}", auth=get_auth_local())
    if response.status_code == 200:
...
def update_article_price(article_id, new_price):
    ...
    response = requests.put(f"{get_localsystem_url()}articles/{article_id}", auth=get_auth_local(), json=update_data)
    if response.status_code == 200:
        return response.json()["data"]
    else:
        return None
